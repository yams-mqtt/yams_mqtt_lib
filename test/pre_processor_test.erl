...
invalid_type_byte_test_() ->
    ...
    [?_assertEqual({error, invalid_fb, <<MsgType:4, Dup:1, QoS:2, Retain:1, 100:8>>}, 
		  (pre_processor:interprete_type_byte(<<MsgType:4, Dup:1, QoS:2, Retain:1, 100:8>>)))
...
interprete_remaining_length_when_packetype_has_error_test_() ->
    TestData = get_test_data(),
    [?_assertEqual({error, invalid_fb, <<MsgType:4, Dup:1, QoS:2, Retain:1, 100:8>>}, 
		  (pre_processor:interprete_remaining_length(pre_processor:interprete_type_byte(<<MsgType:4, Dup:1, QoS:2, Retain:1, 100:8>>))))
     || #testdata{msgtype = MsgType, dup = Dup, qos = QoS, retain = Retain} <- TestData ].
...
interprete_remaining_length_0_test() ->
    ?assertEqual({ok, #type_byte{msgtype = connect, dup =0, qos = 0, retain = 0}, 0, <<>>},
...
interprete_remaining_length_1_test() ->
    ?assertEqual({ok, #type_byte{msgtype = connect, dup =0, qos = 0, retain = 0}, 2, <<100:16>>},
...
interprete_remaining_length_127_test() ->
    ?assertEqual({ok, #type_byte{msgtype = connect, dup =0, qos = 0, retain = 0}, 127, <<100:1016>>},
...
interprete_remaining_length_128_test() ->
    ?assertEqual({ok, #type_byte{msgtype = connect, dup =0, qos = 0, retain = 0}, 128, <<100:1024>>},
...
interprete_remaining_length_16383_test() ->
    ?assertEqual({ok, #type_byte{msgtype = connect, dup =0, qos = 0, retain = 0}, 16383, <<100:131064>>},
...
interprete_remaining_length_16384_test() ->
    ?assertEqual({ok, #type_byte{msgtype = connect, dup =0, qos = 0, retain = 0}, 16384, <<100:131072>>},
...
interprete_remaining_length_2097151_test() ->
    ?assertEqual({ok, #type_byte{msgtype = connect, dup =0, qos = 0, retain = 0}, 2097151, <<100:16777208>>},
...
interprete_remaining_length_2097152_test() ->
    ?assertEqual({ok, #type_byte{msgtype = connect, dup =0, qos = 0, retain = 0}, 2097152, <<100:16777216>>},
...
interprete_remaining_length_268435455_test() ->
    ?assertEqual({ok, #type_byte{msgtype = connect, dup =0, qos = 0, retain = 0}, 268435455, <<100:2147483640>>},
...
interprete_remaining_length_268435456_test() ->
    ?assertEqual({error, remaining_length_exceeds_max_length, {#type_byte{msgtype = connect, dup =0, qos = 0, retain = 0}, <<1:8, 100:2147483648>>}},
...
interprete_packet_test() ->
    ?assertEqual({ok, #type_byte{msgtype = publish, dup =0, qos = 0, retain = 0}, 128, <<100:1024>>},
